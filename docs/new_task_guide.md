...
`indic-llm-eval-pipeline` is a framework that strives to support a wide range of zero- and few-shot evaluation tasks on autoregressive language models (LMs).
...
A more interactive tutorial is available as a Jupyter notebook [here](https://github.com/krutrim-ai-labs/indic-llm-eval-pipeline/blob/main/examples/lm-eval-overview.ipynb).
...
### Tasks with multiple languages

When a task has data in multiple languages (e.g. one benchmark with separate splits or configs per language), you can avoid duplicating shared config by using a **common config file** and **one YAML per language** that include it.

1. **Common config (no `.yaml` extension)**  
   Create a file in the same task directory **without** the `.yaml` extension (e.g. `logprob_indic_copa_common_yaml`). Files without `.yaml` are not discovered as tasks by the harness; they are only loaded when included by another YAML. Put all shared settings there: `dataset_path`, `output_type`, `metric_list`, `doc_to_target`, `doc_to_choice`, `process_docs`, etc. Set `task: null` in the common file so it is not treated as a task.

2. **Per-language YAMLs**  
   For each language, add a normal `.yaml` file (e.g. `logprob_indic_copa_hi.yaml`) that:
   - Uses `include: <common_config_name>` (the basename of the common file, no path) so the common config is merged in.
   - Sets a unique `task: <task_name>` (e.g. `task: logprob_indic_copa_hi`).
   - Overrides only what is language-specific, such as `dataset_name` (e.g. `translation-hi`) or `doc_to_text: !function utils.doc_to_text_hi`.

   The harness looks for the included file in the same directory as the YAML that references it.

3. **Combining and running all languages**  
   To run all language variants together and optionally aggregate metrics, add a **group config** YAML (e.g. `_indic_copa_logprob.yaml`) that lists every per-language task under `task:` and, if desired, defines `aggregate_metric_list` for a single aggregate score. Then run with `--tasks indic_copa_logprob` (or the group name you set).

4. **Generative vs log-prob evaluation**  
   Some tasks support both **log-prob (multiple choice)** and **generative** evaluation on the same data. Keep them as separate task families with separate YAMLs:
   - **Log-prob / multiple choice**: `output_type: multiple_choice`; the model scores each choice and the harness picks the highest. Use metrics like `acc`. Common config and per-language YAMLs can be named e.g. `logprob_<task>_common_yaml` and `logprob_<task>_<lang>.yaml`.
   - **Generative**: `output_type: generate_until`; the model generates text (e.g. "A" or "B") and you match with filters/metrics like `exact_match`. Use a separate common config and per-language YAMLs, e.g. `gen_<task>_common_yaml` and `gen_<task>_<lang>.yaml`.
   Use **separate group configs** for each mode (e.g. `_indic_copa_logprob.yaml` and `_indic_copa_gen.yaml`) so you can run `--tasks indic_copa_logprob` or `--tasks indic_copa_gen` independently. The same dataset and `utils.py` can be shared; only `output_type`, prompts, `generation_kwargs`, `filter_list`, and `metric_list` differ between the two common configs.

**Example (see [`lm_eval/tasks/indic_xtreme/indic_copa/`](../lm_eval/tasks/indic_xtreme/indic_copa/) for the full setup):**

Common config `logprob_indic_copa_common_yaml` (no extension):
...
Per-language YAML `logprob_indic_copa_hi.yaml`:

```yaml
include: logprob_indic_copa_common_yaml
task: logprob_indic_copa_hi
dataset_name: translation-hi
doc_to_text: !function 'utils.doc_to_text_hi'
```

Group config `_indic_copa_logprob.yaml` to run and aggregate all languages:

```yaml
group: indic_copa_logprob
task:
  - logprob_indic_copa_as
  - logprob_indic_copa_bn
  - logprob_indic_copa_hi
  # ... other languages
...
The same `indic_copa` folder also defines a parallel **generative** setup: `gen_indic_copa_common_yaml`, `gen_indic_copa_<lang>.yaml`, and group `_indic_copa_gen.yaml`, so you can run either log-prob or generative evaluation across all languages.
...
If a dataset is straightforward enough, users can enter the feature name directly. This assumes that no preprocessing is required. For example in [Swag](https://github.com/krutrim-ai-labs/indic-llm-eval-pipeline/blob/1710b42d52d0f327cb0eb3cb1bfbbeca992836ca/lm_eval/tasks/swag/swag.yaml#L10-L11), `doc_to_text` and `doc_to_target` given the name of one of the feature each.
...
Hard-coding is also possible as is the case in [SciQ](https://github.com/krutrim-ai-labs/indic-llm-eval-pipeline/blob/1710b42d52d0f327cb0eb3cb1bfbbeca992836ca/lm_eval/tasks/sciq/sciq.yaml#L11).

```yaml
doc_to_target: 3
```

`doc_to_choice` can be directly given a list of text as option (See [Toxigen](https://github.com/krutrim-ai-labs/indic-llm-eval-pipeline/blob/1710b42d52d0f327cb0eb3cb1bfbbeca992836ca/lm_eval/tasks/toxigen/toxigen.yaml#L11))

```yaml
doc_to_choice: ['No', 'Yes']
```

if a dataset feature is already a list, you can set the name of the feature as `doc_to_choice` (See [Hellaswag](https://github.com/krutrim-ai-labs/indic-llm-eval-pipeline/blob/e0eda4d3ffa10e5f65e0976161cd134bec61983a/lm_eval/tasks/hellaswag/hellaswag.yaml#L13))
...
We do this by [writing](https://github.com/krutrim-ai-labs/indic-llm-eval-pipeline/blob/1710b42d52d0f327cb0eb3cb1bfbbeca992836ca/lm_eval/tasks/super_glue/boolq/default.yaml#L9C1-L9C61)
...
We can load this function in `doc_to_target` by using a `!function` operator after `doc_to_target` and followed by `<file name>.<function name>`. In the file [wikitext.yaml](https://github.com/krutrim-ai-labs/indic-llm-eval-pipeline/blob/main/lm_eval/tasks/wikitext/wikitext.yaml) we write:
...
For more detail on the task system and advanced features, see [`docs/task_guide.md`](https://github.com/krutrim-ai-labs/indic-llm-eval-pipeline/blob/main/docs/task_guide.md). If none of the above sounds like they apply to your task, it's time to continue onto checking your task performance!
